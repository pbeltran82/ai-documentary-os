...
class Project(Base):
    ...
    scenes: Mapped[list[Scene]] = relationship(back_populates="project", cascade="all, delete-orphan", order_by="Scene.scene_number")
...
class Scene(Base):
    ...
    animation_plan: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    preferred_asset_type: Mapped[str] = mapped_column(String(40), default="stock_video", nullable=False)
    asset_status: Mapped[str] = mapped_column(String(40), default="missing", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now, nullable=False)

    project: Mapped[Project] = relationship(back_populates="scenes")
    selected_asset: Mapped[Asset | None] = relationship(back_populates="scene", cascade="all, delete-orphan", uselist=False)
...
class Asset(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    scene_id: Mapped[int] = mapped_column(ForeignKey("scenes.id", ondelete="CASCADE"), unique=True, index=True, nullable=False)
