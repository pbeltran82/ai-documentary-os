...
Keeps family detection semantic, applies diversity only inside Tech & Behavior,
cleans the Shorts ending, and removes the 16:9 grid while preserving the
approved renderer architecture.
"""
...
def _score_templates_with_prior(
    ...
    """Prefer unused Tech concepts before visual reuse."""
...
def _suggest_template(scene: object) -> tuple[route.base.TechTemplate, float, str]:
    """Use diversity-aware ranking without polluting family confidence."""
    selected_score, selected = _score_templates_with_prior(
        scene,
    ...
    if selected.template_id != raw_template_id:
        return (
            ...
            f"{route.base.TEMPLATE_BY_ID[raw_template_id].label} in the recent project sequence.",
        )
    if decisive is not None and (
        decisive[0] != route.CTA_TEMPLATE_ID or route.is_terminal_scene(scene)
    ):
        template_id, phrase = decisive
        return (
            route.base.TEMPLATE_BY_ID[template_id],
            0.97,
            f'Decisive scene phrase: “{phrase}”.',
        )
    template, confidence, reason = route.truthful.suggest_template(scene)
...
def _clean_story_cta(canvas: Image.Image, progress: float) -> None:
    ...
    shorts._text(draw, (shorts.SAFE_LEFT, 1190), "YOUR ACTIONS BECOME PREDICTIONS.", 39, thesis_color, bold=True)
...
def _clean_landscape_background(style_id: str, time_seconds: float) -> Image.Image:
    ...
    for y in range(height):
        pixels[0, y] = _blend(top, bottom, y / max(1, height - 1))
    image = gradient.resize((width, height))

    glow = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(glow)
    drift = round(36 * math.sin(float(time_seconds) * 0.24))
    ...
    return Image.alpha_composite(
        image.convert("RGBA"),
        overlay.filter(ImageFilter.GaussianBlur(70)),
    ).convert("RGB")
...
def install_final_pass() -> None:
    ...
    # Family recommendation must stay based on raw semantic relevance. Only the
    # within-Tech template chooser receives project-level diversity penalties.
    route.score_templates = route.truthful.score_templates
    route.suggest_template = _suggest_template
    route.base.score_templates = route.truthful.score_templates
    route.base.suggest_template = _suggest_template
