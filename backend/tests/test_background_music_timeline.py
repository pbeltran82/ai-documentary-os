...
class BackgroundMusicTimelineTests(unittest.TestCase):
    ...
    def test_music_settings_are_clamped_to_safe_ranges(self) -> None:
        normalized = music.normalize_timeline_style({"music_enabled": True, "music_gain_db": -4, "music_ducking_db": -40, "music_fade_seconds": 99})
    ...
    def test_music_filter_loops_to_runtime_with_gain_and_fades(self) -> None:
        chain = music._music_filter(3, 48.7, {"music_gain_db": -24.0, "music_fade_seconds": 0.6}, "music_bed")
    ...
    def test_narration_mix_uses_sidechain_ducking_and_limiter(self) -> None:
        ...
        try:
            graph = music.build_filter_graph([], 30.0, {"music_gain_db": -22.0, "music_ducking_db": -8.0, "music_fade_seconds": 1.5}, voiceover_input_index=9, music_input_index=10)
        finally:
    ...
    def test_music_command_adds_looped_input_and_audio_output(self) -> None:
        original_command = music._original_build_ffmpeg_command
        original_graph = music._original_build_filter_graph
        music._original_build_ffmpeg_command = lambda *args, **kwargs: ["ffmpeg", "-filter_complex", "[v0]null[outv]", "-map", "[outv]", "-an", "output.mp4"]
        music._original_build_filter_graph = lambda *args, **kwargs: "[v0]null[outv]"
        try:
            command = music.build_ffmpeg_command(
                [{"duration_seconds": 10.0}],
                Path("output.mp4"),
                style={"music_enabled": True, "music_gain_db": -22.0, "music_ducking_db": -8.0, "music_fade_seconds": 1.5},
                runtime_seconds=10.0,
                music={"source_file": "/tmp/music.mp3"},
            )
        finally:
            music._original_build_ffmpeg_command = original_command
            music._original_build_filter_graph = original_graph
    ...
    def test_disabled_music_preserves_original_command(self) -> None:
        ...
        try:
            command = music.build_ffmpeg_command([], Path("output.mp4"), style={"music_enabled": False}, music={"source_file": "/tmp/music.mp3"})
        finally:
