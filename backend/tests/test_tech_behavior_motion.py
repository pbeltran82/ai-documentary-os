...
class TechBehaviorMotionTests(unittest.TestCase):
    def scene(self, narration: str, visual_intent: str = "") -> Scene:
        project = Project(id=2, title="The Algorithm Chose You", topic="AI prediction and behavioral modeling", target_minutes=1, audience="General audience", tone="Tense documentary", visual_style="Cinematic technology documentary", status="assets")
        scene = Scene(id=12, project_id=2, scene_number=2, start_seconds=6, end_seconds=12, duration_seconds=6, narration=narration, visual_intent=visual_intent, search_keywords=[], preferred_asset_type="stock_video", asset_status="missing")
        scene.project = project
        project.scenes = [scene]
        return scene

    def project_scenes(self, narrations: list[str]) -> list[Scene]:
        project = Project(id=22, title="Behavioral Ranking", topic="AI prediction and behavioral modeling", target_minutes=1, audience="General audience", tone="Tense documentary", visual_style="Cinematic technology documentary", status="assets")
        project.scenes = [Scene(id=100 + index, project_id=project.id, scene_number=index, start_seconds=(index - 1) * 6, end_seconds=index * 6, duration_seconds=6, narration=narration, visual_intent="", search_keywords=[], preferred_asset_type="stock_video", asset_status="missing") for index, narration in enumerate(narrations, 1)]
        for scene in project.scenes:
            scene.project = project
        return project.scenes

    def test_catalog_exposes_expanded_tech_templates(self) -> None:
        catalog = tech.template_catalog()
        expected = {
            "algorithm_chose_you", "behavior_prediction_engine", "life_event_timeline",
            "digital_footprint_collector", "behavioral_twin", "machine_choice_explainer",
            "attention_auction", "signal_feedback_loop", "profile_forecast",
            "consequence_map", "machine_choice_cta",
        }
        self.assertEqual(len(catalog), len(expected))
        self.assertEqual({item["template_id"] for item in catalog}, expected)

    def test_family_catalog_exposes_three_modular_visual_systems(self) -> None:
        catalog = exact_visuals.family_catalog()
        self.assertEqual(len(catalog), 3)
        self.assertEqual({item["family_id"] for item in catalog}, {exact_visuals.FINANCE_FAMILY_ID, exact_visuals.CHARACTER_FAMILY_ID, exact_visuals.TECH_FAMILY_ID})
    ...
    def test_algorithm_documentary_scenes_route_to_tech_motion(self) -> None:
        ...
        for narration in examples:
            with self.subTest(narration=narration):
                family_id, confidence, reason = exact_visuals.recommend_family(self.scene(narration))
                self.assertEqual(family_id, exact_visuals.TECH_FAMILY_ID)
                self.assertGreaterEqual(confidence, 0.60)
                self.assertTrue("algorithmic behavior" in reason or "Technology, prediction" in reason)

    def test_finance_and_character_regression_routing_remains_intact(self) -> None:
        finance_family, _confidence, _reason = exact_visuals.recommend_family(self.scene("Compound interest grows inside a low-cost S&P 500 index fund."))
        character_family, _confidence, _reason = exact_visuals.recommend_family(self.scene("Most people get paid, pay rent, buy groceries, and go out."))
    ...
    def test_template_recommendations_match_the_script_concepts(self) -> None:
        cases = (
            ...
            ("The highest bidder competes for your attention.", "attention_auction"),
            ("The feedback loop turns the response into the next signal.", "signal_feedback_loop"),
            ("The profile becomes a forecast of future events.", "profile_forecast"),
            ("The ranking shapes what reaches you and your next choice.", "consequence_map"),
    ...
    def test_cta_is_reserved_for_the_terminal_scene(self) -> None:
        scenes = self.project_scenes(["Did you choose, or did the machine choose the moment?", "Did you choose, or did the machine choose the moment? Like and subscribe."])
    ...
    def test_project_sequence_avoids_recent_exact_template_repeats(self) -> None:
        scenes = self.project_scenes([
            ...
            "The highest bidder competes for your attention.",
            "The model changes what reaches you and creates the next signal.",
            "Those signals create a behavioral twin of you.",
            "A profile becomes a forecast of likely outcomes.",
            "A recommendation system ranked the opportunity.",
            "That ranking shapes the next path in front of you.",
    ...
    def test_project_sequence_uses_expanded_variants_before_a_third_use(self) -> None:
        scenes = self.project_scenes(["Those signals create a behavioral twin that predicts you next." for _index in range(10)])
        selected = [tech.suggest_template(scene)[0].template_id for scene in scenes]
        counts = Counter(selected)
        self.assertLessEqual(max(counts.values()), 2)
        self.assertGreaterEqual(len(set(selected)), 6)
        self.assertTrue(set(selected).issubset({
            "behavior_prediction_engine", "digital_footprint_collector", "behavioral_twin",
            "signal_feedback_loop", "profile_forecast", "attention_auction",
            "consequence_map", "life_event_timeline", "algorithm_chose_you",
    ...
    def test_tech_end_card_uses_shared_red_subscribe_and_blue_like_actions(self) -> None:
        ...
        final_bar_bottom = tech.MACHINE_CHOICE_BAR_START_Y + 6 * tech.MACHINE_CHOICE_BAR_STEP_Y + tech.MACHINE_CHOICE_BAR_HEIGHT
    ...
    def test_all_tech_templates_render_distinct_1080p_frames(self) -> None:
        ...
        for template in tech.TEMPLATES:
            with self.subTest(template=template.template_id):
                frame = tech.render_frame(template.template_id, 6, 3.2, tech.DEFAULT_STYLE_ID)
    ...
    def test_all_three_house_styles_work_with_tech_templates(self) -> None:
        frames = [tech.render_frame("behavioral_twin", 6, 3.1, style.style_id) for style in tech.STYLES]
    ...
    def test_storyboard_beats_change_the_rendered_story(self) -> None:
        ...
        frames = [tech.render_frame("behavior_prediction_engine", 6, float(beat["time_seconds"]), tech.DEFAULT_STYLE_ID) for beat in beats]
    ...
    def test_expanded_storyboard_beats_change_the_rendered_story(self) -> None:
        for template_id in ("attention_auction", "signal_feedback_loop", "profile_forecast", "consequence_map"):
            with self.subTest(template=template_id):
                beats = tech.storyboard_beats(template_id, 6)
                frames = [tech.render_frame(template_id, 6, float(beat["time_seconds"]), tech.DEFAULT_STYLE_ID) for beat in beats]
                self.assertEqual(len(beats), 3)
                self.assertIsNotNone(ImageChops.difference(frames[0], frames[-1]).getbbox())

    def test_exports_keep_footer_safe_area_unbranded(self) -> None:
        frame = tech.render_frame("algorithm_chose_you", 6, 3.0, "clean_infographic")
        safe_area = frame.crop((80, 975, 1840, 1040)).convert("L")
        bright_content = safe_area.point(lambda value: 255 if value >= 235 else 0, mode="1")
